...
//静态函数只在本文件使用
//静态函数前置声明
static int DoubleLinkListAccordAppintValGetPos(DoubleLinkList *pList, ELEMENTTYPE val, int* pPos, int (*compareFunc)(ELEMENTTYPE, ELEMENTTYPE));
...
int DoubleLinkListDelAppointData(DoubleLinkList *pList, ELEMENTTYPE val, int (*compareFunc)(ELEMENTTYPE, ELEMENTTYPE))
{
    ...
    while (DoubleLinkListGetLength(pList, &size) && pos != NOT_FIND)
    {
        ...
        DoubleLinkListAccordAppintValGetPos(pList, val, &pos, compareFunc);
